...
for (const lang of Object.keys(language)) {
  if (!validateMnemonic(getMnemonic(12, lang), lang)) {
    console.error("getMnemonic() feaild", lang, 12);
    process.exit(1);
  }
  if (!validateMnemonic(getMnemonic(15, lang), lang)) {
    console.error("getMnemonic() feaild", lang, 15);
    process.exit(1);
  }
  if (!validateMnemonic(getMnemonic(18, lang), lang)) {
    console.error("getMnemonic() feaild", lang, 18);
    process.exit(1);
  }
  if (!validateMnemonic(getMnemonic(21, lang), lang)) {
    console.error("getMnemonic() feaild", lang, 21);
    process.exit(1);
  }
  if (!validateMnemonic(getMnemonic(24, lang), lang)) {
    console.error("getMnemonic() feaild", lang, 24);
    process.exit(1);
  }
}

{
  ...
  const englishPassword = "TREZOR";

  for (const [entropy, mnemonic, seed] of testData.english) {
    if (!validateMnemonic(mnemonic)) {
      console.error("validateMnemonic() failed:", mnemonic);
      process.exit(1);
    }
    if (getMnemonicByEntropy(entropy, language.english) !== mnemonic) {
      console.error("getMnemonicByEntropy() failed", mnemonic);
      process.exit(1);
    }
    if (seed !== toSeedHex(mnemonic, englishPassword)) {
  ...
  for (const [entropy, mnemonic, seed] of testData.japanese) {
    ...
    if (getMnemonicByEntropy(entropy, language.japanese) !== mnemonic) {
      console.error("getMnemonicByEntropy() failed", mnemonic);
      process.exit(1);
    }
